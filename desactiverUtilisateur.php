        <?php
            ...
            $sql = "SELECT idUser FROM users WHERE type = 'admin'"; // On récupère l'id de l'admin
            $req = $db->query($sql);
            $data = $req->fetchAll();
            foreach($data as $row) {
                if($_GET['id'] != null && !in_array($_GET['id'], $row)) { // Si l'id n'est pas null ou que l'id de l'utilisateur est différent de celui de l'admin
                    $id = $_GET['id'];
        ...
        <header class="bg-dark py-5">
            <div class="container px-4 px-lg-5 my-5">
                <div class="text-center text-white">
                    <h1 class="display-4 fw-bolder">Désactivation de l'utilisateur</h1>
                    <p class="lead fw-normal text-white-50 mb-0">Voulez vous désactivez cet utilisateur ?</p>
                </div>
            </div>
        </header>
        ...
        <?php
            ...
            if($_POST) {
                $id = $_POST['iduser'];
                $sql = $db->prepare("UPDATE users SET active = 0 WHERE idUser = $id"); // On désactive l'utilisateur en changeant son statut active à 0
                try {
                    $sql->execute();
