        <?php
            ...
            if(!empty($_GET['id'])) { // Si l'id n'est pas null
                $id = $_GET['id'];
                $sql = 'SELECT * FROM photo WHERE idPhoto = '.$id.' AND idPhoto NOT IN (SELECT idPhoto from acheter)'; // On récupère les données de l'utilisateur
                $req = $db->query($sql);
        ...
        <header class="bg-dark py-5">
            <div class="container px-4 px-lg-5 my-5">
                <div class="text-center text-white">
                    <h1 class="display-4 fw-bolder">Désactivation de la photo</h1>
                    <p class="lead fw-normal text-white-50 mb-0">Voulez vous désactivez cette photo ?</p>
                </div>
            </div>
        </header>
        ...
        <?php
            ...
            if($_POST) {
                $id = $_POST['idphoto'];
                $sql = $db->prepare("UPDATE photo SET active = 0 WHERE idPhoto = $id"); // On désactive la photo en changeant son statut active à 0
                try {
                    $sql->execute();
                    echo '<script>location.href="gererPhoto.php";</script>';
