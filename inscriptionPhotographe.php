<?php
    ...
    if(isset($_POST['submit'])) {
        ...
        if($_FILES) {
            ...
            $sqlmail = 'SELECT count(email) FROM users WHERE email="'.$email.'" AND type="photographe"';
            ...
            // Condition sur le siret
            $sqlsiret = 'SELECT count(siret) FROM users WHERE siret="'.$siret.'" AND type="photographe"';
            $reqsiret = $db->query($sqlsiret);
            $resultsiret = $reqsiret->fetch();
            foreach($resultsiret as $rsiret) {
                if($rsiret == 1) {
                    $ok = false;
                }
            }
            ...
            if($ok == true) {
                $sql = $db->prepare('INSERT INTO users(nom, prenom, email, mdp, siret, active, type, photoUser) VALUES(InitCap(:nom), InitCap(:prenom), lower(:email), :mdp, :siret, 0, "photographe", :photo)');
                ...
                echo '<script>location.href="formInscriptionPhotographe.php";</script>';
            }
        } else {
            echo '<script>location.href="formInscriptionPhotographe.php";</script>';
        }
